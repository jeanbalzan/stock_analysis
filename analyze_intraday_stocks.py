...
def query2():
    query = """
        SELECT time_bucket('{bucket}', dt) AS bucket, sum(trading_volume) AS volume
        FROM stocks_intraday
        WHERE symbol = '{symbol}'
        GROUP BY bucket
        ORDER BY bucket
...
def query3():
    query = """
        SELECT time_bucket('{bucket}', dt) AS bucket,
        last(price_close, dt) AS last_closing_price
        FROM stocks_intraday
        WHERE symbol = '{symbol}'
        GROUP BY bucket
        ORDER BY bucket
...
def query4():
    query = """
        ...
        FROM ( 
            SELECT 
            symbol, 
            time_bucket('{bucket}', dt) AS bucket, 
            first(price_open, dt) AS opening_price, 
            last(price_close, dt) AS closing_price
            FROM stocks_intraday
...
def query5():
    query = """
        SELECT symbol, time_bucket('{bucket}', dt) AS bucket, 
        last(price_close, dt) AS last_closing_price
        FROM stocks_intraday
...
def query8():
    ...
    query = """
        SELECT time_bucket('{bucket}', dt) AS bucket, 
        FIRST(price_open, dt) AS price_open, 
        LAST(price_close, dt) AS price_close,
        MAX(price_high) AS price_high,
        MIN(price_low) AS price_low
        FROM stocks_intraday
        WHERE symbol = '{symbol}' AND date(dt) = date('{date}') 
        GROUP BY bucket
